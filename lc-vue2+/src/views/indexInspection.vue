           ...
           <div class="slide" ref="scrollBar">
               <div class="slideBar">
                   <div class="CarInfo" v-for="car in carList">
                       ...
                       <div class="operate">
                           <label class="label">年检到期提醒</label>
                            <mt-switch @change="change(car)" v-model="car.isAnnualInspectionNotify"></mt-switch>
                       </div>
                       <div class="datePickerWrap iconfont icon-chevron-copy-copy-copy-copy-copy-copy" @click="onOpenAreaPicker(car)">
                           <label>设置年检提醒</label><input type="text" readonly placeholder="请选择" v-model="car.annualInspection">
                       </div>
                   </div>
               </div>
...
export default {
   ...
   methods: {
       confirmDatePicker (val) {
           ...
           this.curCar.annualInspection = date.getFullYear() + '-' + month + '-' + date.getDate()
        //    this.insuranceData = new Date(this.curCar.annualInspection)
           this.insuranceData = this.curCar.annualInspection

           var params = {
               id: this.curCar.id,
               annualInspection: this.insuranceData,
               isAnnualInspectionNotify: this.curCar.isCarInsuranceNotify
           }
           ajax.ajax({
               'vue': this,
           ...
           ajax.ajax({
               ...
               'statusOk': function (res, v) {
                   ...
                   for (var i = 0; i < tmpdata.length; i++) {
                       var date = new Date(tmpdata[i].annualInspection )
                       var month = parseInt(date.getMonth()) + 1
                       var hours = date.getHours() > 8? date.getHours() - 8: 24 + date.getHours() -8
                       tmpdata[i].annualInspection = date.getFullYear() + '-' + month + '-' + date.getDate()
                   }
                   v.carList = tmpdata
                //    setTimeout(function () {
                //        v._initScroll()
