...
export default {
    ...
    methods: {
        closeRechargeRel () {
            this.$emit('closeRechargeInParent')
        },
        select (type) {
            this.payType = type
        },
                ...
                'statusOk': function (res, v) {
                    ...
                    if (ua === 'android') {
                        ...
                        if (v.payType === 1) {
                            ...
                            window.webkit.messageHandlers.HybridKit.postMessage({"method": "pay", "param": {"type":"101","consumeType": "recharge","value":dat}})
                        } else if (v.payType ===2) {
                            var dat = JSON.parse(res.data.data)
                            window.webkit.messageHandlers.HybridKit.postMessage({"method": "pay", "param": {"type":"102","consumeType": "recharge","value":dat}})
                        } else if (v.payType === 4) {
                        } else {
                        }
                    }
