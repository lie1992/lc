<template>
    <div class="headerCommon">
        ...
        <div class="headRight" v-if="rightParams">
            ...
            <a @click="rightPath"><span v-if="rightParams.rightIcon" class="iconfont" :class="rightParams.rightIcon"></span></a>
        </div>
    </div>
</template>
...
<script type="text/ecmascript-6">
    export default {
        ...
        methods: {
            ...
            _goPage (_path) {
                // this.$router.push({path: '/goods/detail', query: {gid: node.name}})
                console.log(_path)
                if (!_path || _path === '') {
                    this.$router.back()
                } else if (_path === 'close') {
