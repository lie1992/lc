<template lang="html">
    <div id="orderConfirm" >
        ...
        <div class="orderConfirmWrap ">
            ...
            <div class="atotal">应付订金：<em>￥{{$store.state.cart.cartConfirmDJ}}</em><button @click="placeOrder()" v-if="showSubmitButtom">提交</button></div>
        </div>
...
export default {
    ...
    created () {
        this.selectArr = this.$store.state.cart.selectArr
    ...
    methods: {
        ...
        getConfirmData () {
            var _selectArr = this.$store.state.cart.selectArr
            var nparam = {
                "storeShoppingCarts": [],
                ...
                // 购物车选中的服务
                "shoppingCartItemIds": _selectArr
            }

            var _selectCartData =  this.$store.state.cart.selectCartData
            var tmp = {}
            for (var i = 0; i < _selectCartData.length; i++) {
                if (_selectCartData[i].serviceTypeId === 2) {
                    tmp = {
                        "storeId": _selectCartData[i].storeId,
                        "couponId": '',
                        "appointment": ''
                    }
        ...
        placeOrder () {
            ...
            for (var i = 0; i < MDdata.length; i++) {
                if (!MDdata[i].YY) {
                    window.$.PageDialog.ok('请选择' + MDdata[i].storeName + '的服务预约时间')
                    return false
                }
