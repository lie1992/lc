...
export default {
    ...
    created () {
        if (this.$store.state.user.userDefaultCar && this.$store.state.user.userDefaultCar.carInsuranceRemind ) {
            console.log(this.$store.state.user.userDefaultCar.carInsuranceRemind )
            var date = new Date(this.$store.state.user.userDefaultCar.carInsuranceRemind )
    ...
    methods: {
        ...
        update () {
            if (!this.insuranceData) {
                this.defaultCar.isCarInsuranceNotify = false
                this.$emit('closeBX')
                return
            }
            var params = {
            ...
            ajax.ajax({
                ...
                'statusOk': function (res, v) {
                    if (res.data.success === true) {
                        ...
                        // v.$router.back()
                        v.$store.state.user.userDefaultCar.carInsuranceRemind  = v.insuranceData
