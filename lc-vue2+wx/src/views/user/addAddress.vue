<template lang="html">
    ...
    <div id="addAddress">
        ...
        <div id="addressWrap">
            ...
            <div class="location" v-if="locationToggle">
                <p>使用当前定位地址：{{locationArea}}</p>
                <p><button @click="locationCancel()">否</button><button @click="locationSure()">是</button></p>
            </div>
            <div class="info">
                <form class="addAddressForm ">
                    <div class=""><label>收货人</label><input type="text" placeholder="请填写" v-model="name"></div>
                    <div class=""><label>手机号</label><input type="text" placeholder="请填写" value="" maxlength='11' v-model="mobile" @keyup="onlySZ"></div>
                    ...
                    <div class="iconfont icon-chevron-copy-copy-copy-copy-copy-copy" @click="onAddJDSel"><label>所属街道</label><input type="text" readonly name="" placeholder="请选择" :value="roadStr"></div>
        ...
        <div class="roadWrap" v-if="roadToggle">
            <div class="roadback"><span class="back iconfont icon-fanhui" @click="closeJD"></span>选择街道</div>
            <div class="roadListWrap">
                <li v-for="item in roadData" @click="setRoadData(item.id, item.name)">{{item.name}}<span class="iconfont icon-chevron-copy-copy-copy-copy-copy-copy"></span></li>
            </div>
        </div>
    </div>
...
export default {
    ...
    created () {
        if (!window.sessionStorage.locationData) {
            this.locationToggle = false
        } else {
            var ar = window.sessionStorage.locationData.split(',')
            this.locationArea = ar[1] + ' ' + ar[2]
        }
    ...
    methods: {
        ...
        onAddJDSel () {
            ...
            if (!this.area) {
                window.$.PageDialog.ok('请先选择地区')
                return false
            }
            ajax.ajax({
                'vue': this,
                'port': 'GetRegionsByParentId',
                'params_url': '?parentId=' + this.area,
                'type': 'get',
                'statusOk': function (res, v) {
                    v.roadData = res.data.data
                },
                'statusError': function (res, v) {
                    window.$.PageDialog.ok('获取地址失败')
                }
            })
            this.roadToggle = true
        },
        onCloseJDSel () {
            this.roadToggle = false
        },
        ...
        onSubmit () {
            ...
            if (!this.name) {
                window.$.PageDialog.ok('收货人不能为空')
                return
            }
            if(!(/^1[34578]\d{9}$/.test(this.mobile))){
                window.$.PageDialog.ok("手机号码输入有误")
                return
            }
            // if (!this.mobile) {
            //     window.$.PageDialog.ok('手机号码不能为空')
            //     return
            // }

            if (!this.areaStr) {
                window.$.PageDialog.ok('请选择地址')
                return
            }
            if (!this.roadID) {
                window.$.PageDialog.ok('请选择街道')
                return
            }
            if (!this.streetDetail) {
                window.$.PageDialog.ok('请填写您的详细地址')
                return
            }
            var params = {
            ...
            ajax.ajax({
                ...
                'statusError': function (res, v) {
                    v.isSubing = false
                    window.$.PageDialog.ok('添加地址失败，请稍后重试')
                }
            })
            // var that = this
        ...
        onAddSel () {
            ...
            //     if (res.data.success === true) {
            //         this.areaList = res.data.data.provinces
            //         setTimeout(function () {
            //             that._initScroll()
            //         }, 300)
            //     } else {
